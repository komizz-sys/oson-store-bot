...
async def get_file_id_wrong_type(message: Message):
  await message.answer(
      "Это не видео. Пришли именно видеофайл (или /cancel)."
  )
...
async def add_gift_start(message: Message, state: FSMContext):
  if not is_admin(message.from_user.id):
    await message.answer(
        ...
        "Если это ты — добавь этот ID в переменную ADMIN_IDS сервиса Worker на"
        " Railway "
...
async def add_gift_id(message: Message, state: FSMContext):
  ...
  if not gift_id.isdigit():
    await message.answer(
        "Нужно просто число, без пробелов и лишних символов. Попробуй ещё раз:"
    )
    return
  await state.update_data(gift_id=gift_id)
  await state.set_state(AddGiftStates.waiting_star_count)
  await message.answer(
      "2) Сколько звёзд этот подарок реально стоит в Telegram? (нужно, чтобы бот"
      " смог его подарить). Например: 50"
  )
...
async def add_gift_price(message: Message, state: FSMContext):
  ...
  await message.answer(
      "4) Теперь пришли сам стикер этого подарка (если он есть в наборе"
      " стикеров или ты можешь его как-то отправить как обычный стикер) — я"
      " возьму из него картинку.\n\nЕсли стикера под рукой нет — напиши /skip,"
      " сохраню без картинки (в магазине будет эмодзи)."
  )


async def _push_gift_to_web(
    data: dict, sticker_bytes: bytes | None, ext: str | None
) -> str:
  """Отправляет данные подарка (и картинку, если есть) на сервис oson-store-web, где физически лежит статика магазина. Возвращает текст для ответа админу."""
  if not config.WEBAPP_URL or not config.INTERNAL_PUSH_SECRET:
    return (
        "⚠️ Не настроен WEBAPP_URL или INTERNAL_PUSH_SECRET — не могу"
        " передать подарок на веб-сервис."
    )
  ...
  return (
      f"✅ Подарок #{data['gift_id']} добавлен в каталог"
      + (
          " с картинкой!"
          if has_image
          else " (без картинки, будет эмодзи 🎁)."
      )
  )
...
async def add_gift_custom_emoji(
    message: Message, state: FSMContext, bot: Bot
):
  """Premium-юзеры часто шлют не обычный стикер, а custom emoji прямо в тексте — достаем реальный стикер через API."""
  data = await state.get_data()
  ...
  if not stickers:
    await message.answer(
        "Не удалось получить картинку из этого эмодзи. Пришли обычный стикер"
        " или /skip."
    )
    return
  sticker = stickers[0]
...
async def admin_help(message: Message):
  ...
  await message.answer(
      ...
      "/addgift — добавить снятый с продажи Telegram-подарок в каталог (с"
      " картинкой)\n"
...
async def broadcast_send(message: Message, state: FSMContext, bot: Bot):
  ...
  status_msg = await message.answer(
      f"⏳ Рассылаю {len(user_ids)} пользователям..."
  )
  sent, failed = 0, 0
...
async def stats_cmd(message: Message):
  ...
  status_lines = (
      "\n".join(
          ...
          for k, v in s["orders_by_status"].items()
      )
      or "  —"
  )
...
async def finalize_payment(bot: Bot, order_id: int, order: dict) -> None:
  """Всё, что должно произойти, когда оплата заказа подтверждена."""
  ...
  if order["category"] == "stars":
    ...
    await start_rent_payment(
        ...
        order,
        order["nft_address"],
        float(order["base_price_per_day_gram"]),
        order["rent_days"],
    )
      ...
      for admin_id in config.ADMIN_IDS:
        try:
          await bot.send_message(
              ...
              " без видео. Отправь мне видео через /getfileid, чтобы это"
              " исправить.",
          )
        except Exception:
          pass
    ...
    if success:
      ...
      await bot.send_message(
          ...
          t(lang, "order_completed").format(
              order_id=order_id, item_name=order["item_name"]
          ),
      )
      await post_completed_order(bot, order)
      await push_live_feed_event(
          FEED_EMOJI.get("simple_gift", "🎁"), order["item_name"]
      )
    for admin_id in config.ADMIN_IDS:
      try:
...
async def reject_payment(call: CallbackQuery, bot: Bot):
  ...
  await set_order_status(
      order_id, "rejected", admin_comment="Оплата не подтверждена"
  )
  await call.message.edit_caption(
      caption=(call.message.caption or "") + "\n\n❌ Отклонено"
  )
...
async def mark_done(call: CallbackQuery, bot: Bot):
  ...
  await post_completed_order(bot, order)
  await push_live_feed_event(
      FEED_EMOJI.get(order["category"], "🎁"), order["item_name"]
  )
