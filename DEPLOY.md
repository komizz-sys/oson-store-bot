...
2. Добавь по одной (New Variable) — значения бери из своего файла `.env`:
   ...
   - `SMS_INCOME_ONLY` = `true` (по умолчанию) — реагировать только на SMS о
     ПОСТУПЛЕНИИ денег. Из-за того, что раньше разбиралась любая SMS с суммой,
     бот писал тебе и про твои обычные покупки в магазинах.
   - `SMS_UNMATCHED_ALERTS` = `false` (по умолчанию) — писать ли админу про
     поступления, не совпавшие ни с одним заказом. Включай только на время
     настройки регулярки под SMS своего банка.
   - `RENT_EXTEND_MIN_DAYS` = `1`, `RENT_EXTEND_MAX_DAYS` = `30` — на сколько
     дней клиент может продлить уже действующую аренду из мини-аппа

3. После сохранения переменных Railway сам перезапустит бота

## Полностью выключить автоподтверждение оплаты по SMS

Поставь `UNIQUE_AMOUNT_ENABLED` = `false` — бот перестанет и выдавать
«уникальные» суммы к оплате, и читать SMS-чат. Оплата дальше подтверждается
кнопкой под чеком, как раньше. Код при этом менять не нужно.
