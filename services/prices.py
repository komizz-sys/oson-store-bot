...
# Сколько звёзд вообще можно купить за один заказ. Это не наша выдумка —
# столько принимает API MarketApp (Fragment), всё вне этого диапазона он
# отбивает с "400: {'detail': '50 - 1,000,000'}". Проверять надо НА СЕРВЕРЕ:
# в витрине количество можно подправить через инструменты разработчика, и
# тогда заказ создаётся, оплата подтверждается, а выполнить его невозможно —
# заказ зависает у клиента, а деньги уже уплачены.
STARS_MIN = 50
STARS_MAX = 1_000_000
