...
# Способ превращения слов в ключ. У 24-словных фраз обычных TON-кошельков это
# "ton" (родной стандарт TON), у 12-словных — обычно "bip39" (тот же стандарт,
# что у биткоина и эфира). Выбор не угадывается: при неверном способе адрес
# получается чужой и пустой. Правильное значение подсказывает /tonwallet —
# он перебирает все варианты и сверяет с твоим адресом.
# Возможные значения: ton, bip39, bip39:1, bip39:2
TON_WALLET_DERIVATION = os.getenv("TON_WALLET_DERIVATION", "ton").strip()
...
# ---- Закупочные цены: сколько товар стоит НАМ ----
# Нужны боту-аналитику, чтобы считать расход и прибыль автоматически, без
# ручного внесения каждой покупки. Держим в переменных окружения, а не в коде:
# поставщик меняет цену — правишь на Railway, передеплоя кода не нужно.
#
# Закупка звезды. Продаём дороже (см. data/prices.json), разница и есть заработок.
STARS_COST_UZS = float(os.getenv("STARS_COST_UZS", "183"))

# Простые подарки: бот дарит их со своего баланса звёзд, поэтому отдельной
# закупочной цены у них нет — считаем, что наценка фиксированная, а всё
# остальное в цене составляет расход.
SIMPLE_GIFT_MARKUP_UZS = int(os.getenv("SIMPLE_GIFT_MARKUP_UZS", "1500"))

# Закупка Premium лежит в data/prices.json рядом с ценой продажи (поле
# cost_uzs у каждого тарифа): тарифов пять, отдельными переменными было бы
# неудобно, а в прайсе они и так все перед глазами.
...
# Границы срока ПРОДЛЕНИЯ уже действующей аренды (в днях). У каждого лота на
# MarketApp свои min/max, но лот, который сейчас в аренде, в каталоге не
# отдаётся — поэтому для продления используем эти общие рамки, а фактическую
# транзакцию всё равно подтверждает админ в кошельке.
RENT_EXTEND_MIN_DAYS = int(os.getenv("RENT_EXTEND_MIN_DAYS", "1"))
RENT_EXTEND_MAX_DAYS = int(os.getenv("RENT_EXTEND_MAX_DAYS", "30"))
...
# Реагировать ТОЛЬКО на SMS о ПОСТУПЛЕНИИ денег (пополнение/приход/kirim).
# БЫЛО: бот разбирал вообще любую SMS с суммой — включая твои собственные
# покупки в магазинах ("Оплата 45 000 сум, Korzinka") — и на каждую такую SMS
# писал админу "сумма не совпала ни с одним заказом". Именно из-за этого
# приходили уведомления при покупках в обычных магазинах.
SMS_INCOME_ONLY = os.getenv("SMS_INCOME_ONLY", "true").lower() == "true"

# Писать ли админу про SMS, которые не совпали ни с одним заказом.
# По умолчанию ВЫКЛЮЧЕНО — это был источник спама. Включай только на время
# настройки регулярки под формат SMS своего банка.
SMS_UNMATCHED_ALERTS = os.getenv("SMS_UNMATCHED_ALERTS", "false").lower() == "true"

# Последние 4 цифры карты магазина (например 2726). Если задано, бот реагирует
# только на поступления ИМЕННО на эту карту — нужно, когда в один чат сыплются
# уведомления по нескольким картам (личная + магазинная), иначе поступление на
# личную карту могло бы случайно совпасть по сумме с чьим-то заказом.
SMS_CARD_LAST4 = os.getenv("SMS_CARD_LAST4", "").strip()
